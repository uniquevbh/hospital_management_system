...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('HMS_SECRET_KEY', 'dev-key')
...
app.config['MAIL_USERNAME'] = os.environ.get('HMS_MAIL_USERNAME', '')  # Update with your email
app.config['MAIL_PASSWORD'] = os.environ.get('HMS_MAIL_PASSWORD', '')     # Update with your app password
...
def test():
    if request.method == 'POST':
        ...
        if (num1 and num2 ):
            ...
            flash('calculated successful!', 'success')
            return redirect(url_for('page3.html'))
        else:
            flash('Invalidnumber')
